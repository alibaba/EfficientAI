...
class ActivationModule:
    ...
    def save_D_and_inv_D(self, D, inv_D):
        os.makedirs(self.file_path, exist_ok=True)
        torch.save(D, f"{self.file_path}/D.pt")
        torch.save(inv_D, f"{self.file_path}/inv_D.pt")
    
    def load_D_and_inv_D(self):
        D = torch.load(f"{self.file_path}/D.pt")
        inv_D = torch.load(f"{self.file_path}/inv_D.pt")
        return D, inv_D
    
    def save_Q1_and_inv_Q1(self, D, inv_D):
        os.makedirs(self.file_path, exist_ok=True)
        torch.save(D, f"{self.file_path}/Q1.pt")
        torch.save(inv_D, f"{self.file_path}/inv_Q1.pt")

    def load_Q1_and_inv_Q1(self):
        D = torch.load(f"{self.file_path}/Q1.pt")
        inv_D = torch.load(f"{self.file_path}/inv_Q1.pt")
        return D, inv_D

    def save_Q2_and_inv_Q2(self, D, inv_D):
        os.makedirs(self.file_path, exist_ok=True)
        torch.save(D, f"{self.file_path}/Q2.pt")
        torch.save(inv_D, f"{self.file_path}/inv_Q2.pt")

    def load_Q2_and_inv_Q2(self):
        D = torch.load(f"{self.file_path}/Q2.pt")
        inv_D = torch.load(f"{self.file_path}/inv_Q2.pt")
        return D, inv_D
...
def get_sparse_model(model_name, device, histogram_path, **kwargs):
    from inference.model import LlamaSparseForCausalLM, MistralSparseForCausalLM, LlamaSparseConfig, MistralSparseConfig, Qwen2SparseConfig, Qwen2SparseForCausalLM
    ...
    assert class_name in ["Qwen2ForCausalLM", "Qwen2SparseForCausalLM", "LlamaForCausalLM", "MistralForCausalLM", "LlamaSparseForCausalLM", "MistralSparseForCausalLM"], f"Model class name {class_name} not supported"
    ...
    if device == 'auto':
        # multi gpu
        return SparseModel.from_pretrained(model_name, torch_dtype=torch.bfloat16, device_map="auto", attn_implementation="flash_attention_2", histogram_path=histogram_path, **kwargs)
    else:
        return SparseModel.from_pretrained(model_name, torch_dtype=torch.bfloat16, device_map=device, attn_implementation="flash_attention_2", histogram_path=histogram_path, **kwargs)

def get_sparse_model_rotate(model_name, device, histogram_path, rot, **kwargs):
    ...
    if device == 'auto':
        # multi gpu
        model = LlamaSparseForCausalLMRotate.from_pretrained(model_name, torch_dtype=torch.bfloat16, device_map="auto", attn_implementation="flash_attention_2", histogram_path=histogram_path, rot=rot, **kwargs)
        return model
        ...
        model = LlamaSparseForCausalLMRotate.from_pretrained(model_name, torch_dtype=torch.bfloat16, device_map=device, attn_implementation="flash_attention_2", histogram_path=histogram_path, rot=rot, **kwargs)
        return model
