...
def top_k_new(x, sparsity_level):
    # Determine the number of elements to keep
    k = int((1 - sparsity_level) * x.shape[-1])
...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            top_k_x = torch.matmul(tmp_x, self.Q.T.to(x.device))
            top_k_x = top_k_x.to(x.dtype)
            # top_k_x = x

            # down_proj = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
            intermediate_states = self.act_fn(self.gate_proj(top_k_x)) * self.up_proj(top_k_x)
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # query_states_sparse = self.q_proj(sparse_x)
        # key_states_sparse = self.k_proj(sparse_x)
        # value_states_sparse = self.v_proj(sparse_x)

        # query_states= self.q_proj(hidden_states)
        # key_states = self.k_proj(hidden_states)
        # value_states = self.v_proj(hidden_states)

        # cos_sim_attn = cosine_similarity(query_states_top_k, query_states)
        # cos_sim_attn_self = cosine_similarity(query_states_sparse, query_states)

        # print(cos_sim_attn, cos_sim_attn_self)
