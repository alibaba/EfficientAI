...
from .modeling_llama_moi import LlamaForCausalLM
...
class MOI_SFTTrainer(SFTTrainer):
    ...
    def _create_model_from_path(self, model_path: str, args: SFTConfig) -> PreTrainedModel:
        ...
        model = LlamaForCausalLM.from_pretrained(model_path, **model_init_kwargs)
    ...
    def map_prepare_packing_mask_dataset_only_last_assistant(
        ...
        roles = {"system": "<|begin_of_text|><|start_header_id|>system<|end_header_id|>", "user": "<|start_header_id|>user<|end_header_id|>", "assistant": "<|start_header_id|>assistant<|end_header_id|>", "tool": "<|start_header_id|>ipython<|end_header_id|>"}

        bos = [128000]
        header_start = processing_class('<|start_header_id|>').input_ids[1:]
        header_end = processing_class('<|end_header_id|>').input_ids[1:]
        eot = processing_class('<|eot_id|>').input_ids[1:]
        nl_tokens = processing_class('\n').input_ids[1:]
        ...
        for conversations, single_conv_last_idx in zip(examples['conversations'], examples['single_conv_last_idx']):
            ...
            for j, sentence in enumerate(conversations):
                if sentence['role'] in roles.keys():
                    ...
                    role = "<|start_header_id|>" + sentence['role'] + "<|end_header_id|>"
                _input_id = processing_class(role).input_ids[1:] + nl_tokens + nl_tokens + \
                    processing_class(sentence["content"]).input_ids[1:] + eot
                input_id += _input_id
                if role == '<|start_header_id|>user<|end_header_id|>':
                    _target = header_start + [ignor_token_id] + header_end + [ignor_token_id]* (len(_input_id)-4) + eot
                    _weight = [1.0] + [0.0] + [1.0] + [0.0] * (len(_input_id)-4) + [1.0]
                    for pos in range(start_pos_id, start_pos_id + len(_input_id)):
                        position_id.append(pos)
                    start_pos_id += len(_input_id) 

                elif role == '<|begin_of_text|><|start_header_id|>system<|end_header_id|>':
                    _target = [ignor_token_id] + header_start + [ignor_token_id] + header_end + [ignor_token_id]* (len(_input_id)-5) + eot
                    _weight = [0.0] + [1.0] + [0.0] + [1.0] + [0.0] * (len(_input_id)-5) + [1.0]
                    if j == 0:
                        last_mask_indice = 1
                    else:
                        last_mask_indice = len(target)
                    ...
                    if j in single_conv_last_idx:
                        _target = header_start + [ignor_token_id] + header_end + _input_id[3:-1] + eot
                    else:
                        _target = header_start + [ignor_token_id] + header_end + [ignor_token_id]* (len(_input_id)-4) + eot
                    
                    _weight = [1.0] + [0.0] + [1.0] + [weight_cur] * (len(_input_id)-4) + [1.0]
                    for pos in range(start_pos_id, start_pos_id + len(_input_id)):
                        position_id.append(pos)
                    start_pos_id += len(_input_id) 

                else:
                    _target = header_start + [ignor_token_id] * (len(processing_class(role).input_ids[1:]) - 2) + header_end + [ignor_token_id]* (len(_input_id)-3-(len(processing_class(role).input_ids[1:]) - 2)) + eot
                    _weight = [1.0] + [0.0] + [1.0] + [0.0] * (len(_input_id)-4) + [1.0]
                    for pos in range(start_pos_id, start_pos_id + len(_input_id)):
                        position_id.append(pos)
                    start_pos_id += len(_input_id) 
            ...
            assert len(input_id) == len(target), print("len of input_id and target not match")
            assert len(input_id) == len(weight), print("len of input_id and weight not match")
            assert len(input_id) == len(flash_attention_mask), print("len of input_id and flash_attention_mask not match")
            position_id += [k for k in range(len(input_id), max_length)]
            input_id += [processing_class.eos_token_id] * (max_length - len(input_id))
            target += [ignor_token_id] * (max_length - len(target))
            weight += [0.0] * (max_length - len(weight))
            flash_attention_mask += [0] * (max_length - len(flash_attention_mask))
        ...
        return dict(
            ...
            labels=targets,
            weights=weights,
            attention_mask=input_ids.ne(processing_class.eos_token_id),
            position_ids = position_ids,
            flash_attention_mask=flash_attention_masks,
        )
